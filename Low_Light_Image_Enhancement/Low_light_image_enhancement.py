...
parser.add_argument('--input_dir', dest='InputPath',
    default=r'D:\ZeDuSR\zedusr\Low_Light_Image_Enhancement\Input_images',
    help='directory for testing inputs')

parser.add_argument('--output_dir', dest='OutputPath',
    default=r'D:\ZeDuSR\zedusr\Low_Light_Image_Enhancement\results',
    help='directory for testing outputs')

parser.add_argument('--gamma_max', dest='gamma_max', type=float, default=6.0,
    help='gamma_max value')
...
def GetMin(fi):
    return cv2.max(cv2.min(fi[:, :, 0], fi[:, :, 1]), fi[:, :, 2])
...
def EstimateTransmission(InputImg, NormImg, gamma_max=5):
    T_min = 0.1
...
if __name__ == "__main__":
    ...
    OutputPath = os.path.join(args.OutputPath, "Output", FolderName)

    os.makedirs(OutputPath, exist_ok=True)
    FileList = [
        file for file in os.listdir(args.InputPath)
        if file.lower().endswith((".jpg", ".jpeg", ".webp", ".tiff", ".tif", ".bmp", ".png"))
    ]

    for FileName in FileList:

        FilePathName = os.path.join(args.InputPath, FileName)
        ...
        OutputFile = os.path.join(OutputPath, Name + ".png")
        cv2.imwrite(OutputFile, f2i(OutputImg))
