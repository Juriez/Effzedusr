...
def _load_vgg16():
    """Load an ImageNet-pretrained VGG16.

    Order of preference:
      1) $VGG16_WEIGHTS env var, if it points to a file
      2) a local ``preTrained/vgg16-397923af.pth`` checkpoint in the repo
      3) torchvision's downloadable weights (needs network on first run)
    """
    model = vgg16()
    here = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.environ.get("VGG16_WEIGHTS", ""),
        os.path.abspath(os.path.join(here, "..", "..", "preTrained", "vgg16-397923af.pth")),
        os.path.abspath(os.path.join(here, "..", "preTrained", "vgg16-397923af.pth")),
    ]
