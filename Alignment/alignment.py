...
class Trainer(object):
    ...
    def training(self, iteration):
        ...
        if self.iteration % args.eval_interval == 0 and self.iteration>10: #200
...
#         current_loss = trainer.train_loss_warp_total
...
#             print(f"\n[Early Stopping] No improvement for {args.patience} iterations. Stopping at iteration {iteration}.")
#             break

#       # Always save PatchDisOut.npy at the end of training (even if early stopped)
#     output_dir = os.path.join(args.output_path, args.dataset)
#     os.makedirs(output_dir, exist_ok=True)
    
#     patch_dis_out = trainer.D_fake_spa.squeeze().detach().cpu().numpy()
#     np.save(os.path.join(output_dir, "PatchDisOut.npy"), np.array(patch_dis_out))

#     print(f"[Saved] PatchDisOut.npy -> {os.path.join(output_dir, 'PatchDisOut.npy')}")
