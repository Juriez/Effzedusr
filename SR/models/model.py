...
import os
# pretrained_path='SR/models/preTrained/RCAN_BIX4.pt'
# Default to None to avoid trying to load a missing absolute path
...
class MODEL(nn.Module):
    def __init__(self, args, pretrained_path=pretrained_path, device = device):
        ...
        # Load pretrained weights if path is given and file exists
        if pretrained_path is not None:
            ...
            if os.path.exists(pretrained_path):
                ...
                print(f"[WARNING] Pretrained path '{pretrained_path}' not found. Skipping pretrained load.")
        else:
